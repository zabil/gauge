...
var gaugeVersion = flag.Bool([]string{"v", "-version", "version"}, false, "Print the current version and exit. e.g., gauge --version")
var verbosity = flag.Bool([]string{"-verbose"}, false, "Enable step level reporting on console, default being scenario level. e.g., gauge --verbose specs")
...
var initialize = flag.String([]string{"-init"}, "", "Initializes project structure in the current directory. e.g., gauge --init java")
var installPlugin = flag.String([]string{"-install"}, "", "Downloads and installs a plugin. e.g., gauge --install java")
var uninstallPlugin = flag.String([]string{"-uninstall"}, "", "Uninstalls a plugin. e.g., gauge --uninstall java")
var installAll = flag.Bool([]string{"-install-all"}, false, "Installs all the plugins specified in project manifest, if not installed. e.g., gauge --install-all")
var update = flag.String([]string{"-update"}, "", "Updates a plugin. e.g., gauge --update java")
...
var installZip = flag.String([]string{"-file", "f"}, "", "Installs the plugin from zip file. This is used with --install. e.g., gauge --install java -f ZIP_FILE")
...
var executeTags = flag.String([]string{"-tags"}, "", "Executes the specs and scenarios tagged with given tags. e.g., gauge --tags tag1,tag2 specs")
var tableRows = flag.String([]string{"-table-rows"}, "", "Executes the specs and scenarios only for the selected rows. e.g., gauge --table-rows \"1-3\" specs/hello.spec")
var apiPort = flag.String([]string{"-api-port"}, "", "Specifies the api port to be used. e.g., gauge --daemonize --api-port 7777")
var refactorSteps = flag.String([]string{"-refactor"}, "", "Refactor steps. e.g., gauge --refactor <old step> <new step> [[spec directories]]")
...
var strategy = flag.String([]string{"-strategy"}, "lazy", "Set the parallelization strategy for execution. Possible options are: `eager`, `lazy`. e.g., gauge -p --strategy=\"eager\"")
var doNotRandomize = flag.Bool([]string{"-sort", "s"}, false, "Run specs in Alphabetical Order. e.g., gauge -s specs")
var validate = flag.Bool([]string{"-validate", "#-check"}, false, "Check for validation and parse errors. e.g., gauge --validate specs")
var updateAll = flag.Bool([]string{"-update-all"}, false, "Updates all the installed Gauge plugins. e.g., gauge --update-all")
var checkUpdates = flag.Bool([]string{"-check-updates"}, false, "Checks for Gauge and plugins updates. e.g., gauge --check-updates")
var listTemplates = flag.Bool([]string{"-list-templates"}, false, "Lists all the Gauge templates available. e.g., gauge --list-templates")
...
var runFailed = flag.Bool([]string{"-failed"}, false, "Run only the scenarios failed in previous run. e.g., gauge --failed")
var docs = flag.String([]string{"-docs"}, "", "Generate documenation using specified plugin. e.g., gauge --docs <plugin name> specs/")
